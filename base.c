...
#define SHIFT_IDX 3
#define U_ONE 1ull
#define __broadcast_byte(x) vdup_n_u8((uint8_t)(x))

static __inline__ __attribute__((always_inline, pure)) uint64_t __testeq_fn_neon(const uint8x8_t fv, const uint8x8_t mask)
{
  return vget_lane_u64(vreinterpret_u64_u8(vceq_u8(fv, mask)), 0) & 0x8080808080808080;
}

#define __testeq__(v, x) __testeq_fn_neon(vld1_u8((void *)*(v)), x)
...
static __inline__ __attribute__((always_inline, pure)) uint64_t __testeq_fn(uint64_t fv, const uint64_t mask)
{ 
  fv ^= mask;
  return __test_zero_wi(fv);
}
...
static __attribute__((nonnull)) void *__find__(const struct __object const *object, const char *__key, uint8_t return_Policy)
{
    ...
    for (uint32_t i = (ROUNDUP_MULP(__size__(object), RDWORD) - where) >> RDWORD_P2; i--;)
    {
        ...
        while (mask)
        {
            where = __scan_reverse(mask) >> SHIFT_IDX;
            if (__compare_hash(tag[where].hash, hash) && NOT(strcmp(tag[where].key, __key)))
                return where;
	    mask &= mask - 1;
        }
        tag = tag + RDWORD;
